...
export const updateProductData = (productData) =>{
    return(dispatch, getState, {getFirebase, getFirestore}) =>{
        ...
        firestore.collection(productData.collection).doc(productData.productid).update({
            ...productData
        }).then(()=>{
...
export const addImages = (product) =>{
    return (dispatch, getState,{getFirebase, getFirestore} ) => {
        ...
        imageArray.map((img,index)=>{
            const uploadtask = storage.ref(product.collection.stringValue+'/'+product.productid.stringValue+'-'+product.productname.stringValue+'/'+img.name).put(img);
            uploadtask.on(
                "state_changed",
                snapshot => {
                ...
                ()=>{
                    storage
                    .ref(product.collection.stringValue+'/'+product.productid.stringValue+'-'+product.productname.stringValue)
                    .child(img.name)
                    .getDownloadURL()
                    .then(url=>{
                        // debugger
